...
namespace RepositaryPattern_With_UnitOfWork.EntityFramework.Repositary
{
    class UnitOfWork
    {
            ...
            where TDbContext : DbContext, new()
        {
            ...
            [ThreadStatic]
            private static ScopedDbContext _scopedDbContext;

            private bool isRoot = false;

            private bool _saveChangesCalled = false;
            ...
            public TDbContext DbContext
            {
                get
                {
                    return _scopedDbContext.DbContext;
                }
            }
            ...
            public UnitOfWorkScope(UnitOfWorkScopePurpose purpose)
            {
                ...
                if (_scopedDbContext == null)
                {
                    _scopedDbContext = new ScopedDbContext(purpose == UnitOfWorkScopePurpose.Writing);
                    isRoot = true;
                }
                if (purpose == UnitOfWorkScopePurpose.Writing && !_scopedDbContext.ForWriting)
                {
                    throw new InvalidOperationException(
            ...
            protected override void Dispose(bool disposing)
            {
                if (disposing)
                {
                    ...
                    if (purpose == UnitOfWorkScopePurpose.Writing && !_saveChangesCalled)
                    {
                        _scopedDbContext.BlockSave = true;
                        // Don't throw here - it would mask original exception when exiting
                        // a using block.
                    }

                    if (_scopedDbContext != null && isRoot)
                    {
                        _scopedDbContext.Dispose();
                        _scopedDbContext = null;
                    }
                }
                base.Dispose(disposing);
            }
            ...
            public void SaveChanges()
            {
                ...
                _scopedDbContext.AllowSaving = true;
                _scopedDbContext.DbContext.SaveChanges();
                _scopedDbContext.AllowSaving = false;
            }
        }
    }
}
